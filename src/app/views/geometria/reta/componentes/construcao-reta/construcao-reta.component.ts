...
@Component({
  selector: 'app-construcao-reta',
  templateUrl: './construcao-reta.component.html',
  styleUrls: ['./construcao-reta.component.scss']
})
export class ConstrucaoRetaComponent implements OnInit {
  ...
  render() {
    ...
    const self: ConstrucaoRetaComponent = this;
  ...
  onMouseClick(event) {
    if(event.toElement.tagName ==='CANVAS'){
      ...
      if (intersects.length === 0) {
        ...
        if ((this.click % 2) === 0) {
          this.ponto1 = this.adicionaPonto(xDoMouse * 5, yDoMouse * 5, this.alfabeto[this.n % 26] + p);
        } else {
          this.ponto2 = this.adicionaPonto(xDoMouse * 5, yDoMouse * 5, this.alfabeto[this.n % 26] + p);
        }
        this.n++;
      } else {
        if ((this.click % 2) === 0) {
