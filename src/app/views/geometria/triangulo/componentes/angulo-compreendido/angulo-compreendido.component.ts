...
@Component({
  ...
  templateUrl: './angulo-compreendido.component.html',
  styleUrls: ['./angulo-compreendido.component.scss']
})
export class AnguloCompreendidoComponent implements OnInit,OnDestroy {
  public formModel: FormGroup = this.fb.group({
    anguloVerticeA: [45, [Validators.required, Validators.min(1)]],
    anguloVerticeB: [45, [Validators.required, Validators.min(1)]],
  ...
  render() {
    const self: AnguloCompreendidoComponent = this;
  ...
  public isTriangle() {
    const a: number = this.formModel.get('anguloVerticeA').value * 1;
    const b: number = this.formModel.get('anguloVerticeB').value * 1;
    if (a + b < 180) {
      this.notTriangle = '';
      this.formModel.get('isTriangle').setValue('true');
      return true;
  ...
  public acharPontos() {
    ...
    const anguloB = this.anguloVerticeA * (Math.PI / 180);
    const anguloC = this.anguloVerticeC * (Math.PI / 180);
    const anguloA = this.anguloVerticeB * (Math.PI / 180);
    a = (c / Math.sin(anguloC)) * Math.sin(anguloA);
    b = (c / Math.sin(anguloC)) * Math.sin(anguloB);

    const x = this.cordBX - (Math.cos(anguloA) * b);
    const y = Math.sin(anguloA) * b + this.cordBY;
  ...
  public atualizarBaseAltura(): void {
    const distancia = this.camera.position.z + 10;
    let curva = new THREE.SplineCurve([new THREE.Vector2(-distancia, 0),
    new THREE.Vector2(distancia, 0)]);
